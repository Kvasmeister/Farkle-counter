...
| 14-nastaveni | rozdělení okna na karty Obecné a Herní režimy včetně návratu na první po zavření, seznam, detail režimu a editor vlastní kombinace jako tři podstránky druhé karty, přepínače nad harmonikou v pořadí jazyk a ukládání, tři oddíly a jejich názvy, výlučnost rozklikávání, sbalení při otevření okna, počty v hlavičkách, údaj o místě jen na hlavní úrovni první karty, celek počítaný sám při otevření okna a rozpis až po rozbalení tlačítkem Detail, jeho šest hodnot včetně oddělených košů a zbylých klíčů `farkle-*`, zabalení druhým klepnutím, chování bez `estimate()`, velikost aplikace sečtená jen z vlastních cache, stavové popisky, obnova z rozbaleného oddílu | 88 |
