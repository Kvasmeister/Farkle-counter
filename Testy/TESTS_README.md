...
| 04-statistiky | všech 31 statistik proti ručně spočítaným číslům (včetně Nejlepšího/Nejhoršího kola a Nejvíc bodů ztraceno farklem rozdělených na celkem/body/kola), jejich pořadí a rozdělení do pěti nadepsaných kategorií (Obecné/Hry/Kola/Hody/Farkly), kola v jedné hře na body jen z dokončených her, seskupení po dnech přes půlnoc, žebříčky včetně žebříčku dnů, typ hry v podřádku i v řádku žebříčku, proklik ze žebříčku do detailu hry a návrat zpátky do něj, detail hry, mazání a návrat z koše pod stejným id | 97 |
...
| 10-uloziste | migrace do IndexedDB, propad bez ní, příznak `idb` bez dostupné IDB, návrat paměti při selhání zápisu, upgrade na dvě police, shoda všech pětadvaceti statistik nad souhrny a nad plnými záznamy včetně nejhranějšího režimu, shodný export z obou úložišť, pole `hodu`/`ztraceno`/`nejlepsihod`/`bodyHodu`/`hoduCelkem` v souhrnu a jejich zpětný dopočet při upgradu (`IDB_VERZE` 6), nula uložená dřív, která se čte jako `null`, export z IndexedDB nesoucí `rezim` i `rezimN` (a jejich návrat při importu téže zálohy), dopočet hodových statistik po doimportování chybějícího režimu ze zálohy včetně obnovy `HIST` bez restartu | 105 |
...
| 15-filtry | filtr jednoho dne i rozsahu, hranice půlnoci, prohození obrácených krajů, přepočet statistik pod filtrem, přežití přepnutí karty, prázdné stavy, záloha filtrem nedotčená, předvyplnění okna, proklik ze žebříčku dnů, přestavění seznamu při změně filtru, lišta podle karty i krátkost jejích popisků, plné znění filtru v `aria-label`, skrytí pole Do mimo režim rozsahu, nabídka typů skládaná z dat i pod filtrem data, filtr typu včetně her bez limitu a jeho vypnutí, statistiky (starý) filtr typu ignorují, čtyři směry řazení, druhotné řazení při shodě bodů, zmizení dělících čar, reset filtrů i řazení včetně popisku tlačítka — a nový, trvalý filtr Statistik podle herního režimu a typu hry: lišta podle karty přesně opačně než u Historie, nabídka režimů z dat včetně smazaného vlastního režimu pod uloženým jménem i živého pod dnešním, plošný dopad i na početní statistiky, nezávislost na starém filtru Historie oběma směry, přežití „restartu“ appky přes localStorage na rozdíl od filtru Historie, který resetuje, a Reset mažící i tenhle filtr, schování rozdělených statistik pro opačný typ hry (i minkol/maxkol, které mají typ vždy) beze ztráty nadpisu kategorie, „celkem“ varianta zůstává napříč oběma typy hry i pod zapnutým filtrem typu (na rozdíl od nedělených statistik, které se filtrem typu dál zužují) | 130 |
