...
| 04-statistiky | všech 21 statistik proti ručně spočítaným číslům, jejich pořadí, kola v jedné hře na body jen z dokončených her, seskupení po dnech přes půlnoc, žebříčky včetně žebříčku dnů, typ hry v podřádku i v řádku žebříčku, proklik ze žebříčku do detailu hry a návrat zpátky do něj, detail hry, mazání a návrat z koše pod stejným id | 89 |
...
| 10-uloziste | migrace do IndexedDB, propad bez ní, příznak `idb` bez dostupné IDB, návrat paměti při selhání zápisu, upgrade na dvě police, shoda všech jednadvaceti statistik nad souhrny a nad plnými záznamy včetně nejhranějšího režimu, shodný export z obou úložišť, nová pole `hodu` a `ztraceno` v souhrnu, jejich zpětný dopočet při upgradu a nula uložená dřív, která se čte jako `null` | 90 |
...
| 21-sdileni-rezimu | výběr ke sdílení přímo v hlavním seznamu režimů — řádek se v tomhle stavu zúží na jedno tlačítko místo Pravidla/Upravit/Zvolit a po dokončení nebo zrušení se vrátí zpátky, sdílení jednoho i víc vybraných režimů (i přednastaveného, s materializovaným jménem) pod vlastním markerem `#SDILENIREZIMU:`, prázdný výběr nic nevygeneruje, import odlišného režimu beze změny jména, shoda jména u jinak odlišných pravidel vede k přejmenování s příponou „(IMPORT - datum)” bez uříznuté závorky, funkční shoda s vlastním i s přednastaveným režimem dávku odmítne bez ohledu na jméno, dávka se smíšenými výsledky přijme jen to, co má, samé duplicity nepřidají nic, otevření nastavení oba panely (patičku i box importu) zavře a vynuluje výběr, tlačítko Importovat se druhým kliknutím zavře stejně jako se otevřelo, výběr ke sdílení a import se navzájem vylučují, tlačítko Přidat vlastní režim je při rozjetém výběru zamčené | 41 |

Po zásahu do `src/` nebo `sw.js` spusť `npm test` — všech dvaadvacet sad.
Dohromady je to 1 543 kontrol.
